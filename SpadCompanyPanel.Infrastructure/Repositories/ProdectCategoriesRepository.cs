...
namespace SpadCompanyPanel.Infrastructure.Repositories
{
    public class ProdectCategoriesRepository : BaseRepository<ProductCategory, MyDbContext>
    {
        ...
        public List<ProductCategory> GetAllProductCategories()
        {
            ...
            {
                category.Products = _context.Products.Where(p => p.ProductCategoryId == category.Id & p.IsDeleted == false).ToList();

                //category.Products.Concat(products);
            }
        ...
        {
            return _context.ProductCategories.Where(pc => pc.IsDeleted == false && pc.Id == id).Include(pc => pc.Products).FirstOrDefault();
        }
